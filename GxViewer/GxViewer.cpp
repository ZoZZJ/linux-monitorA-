...
CGxViewer::CGxViewer(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CGxViewer),
    m_pobjAcqThread(nullptr),
    m_pobjROISettings(nullptr),
    m_pobjFrameRateControl(nullptr),
    m_pobjExposureGain(nullptr),
    m_pobjWhiteBalance(nullptr),
    m_pobjImgProc(nullptr),
    m_pobjUserSetCtl(nullptr),
    m_hDevice(nullptr),
    m_pstBaseInfo(nullptr),
    m_ui32DeviceNum(0),
    m_bOpen(false),
    ...
    m_pobjShowImgTimer(nullptr),
    m_pobjShowFrameRateTimer(nullptr)
{
    ui->setupUi(this);
    ...
    connect(m_pobjShowImgTimer, &QTimer::timeout, this, &CGxViewer::slotShowImage);
    connect(m_pobjShowFrameRateTimer, &QTimer::timeout, this, &CGxViewer::slotShowFrameRate);

    // Connect image save signal and slot non-blocking
    connect(this, &CGxViewer::SigSaveImage, this, &CGxViewer::slotSaveImageFile, Qt::QueuedConnection);
...
*/
//----------------------------------------------------------------------------------
void CGxViewer::StopAcquisition_clicked_slot(){
    //关闭相机并停止采集
//    on_StopAcquisition_clicked();
//    on_CloseDevice_clicked();

}
//开启相机并且开始采集
void CGxViewer::StartAcquisition_clicked_slot(){
    //on_OpenDevice_clicked();
    //on_StartAcquisition_clicked();
