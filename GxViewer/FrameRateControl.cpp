    ...
    m_dFrameRate(0.0)
{
    ...
    /*QAbstractSpinBox 的键盘跟踪功能会在用户输入时立即更新值。禁用此功能后，
    值的更新将在用户按下 Enter 键时或通过其他方式明确触发，这样可以更好地控制输入行为，防止在快速输入时产生不必要的更新。*/
    QObjectList pobjGroupList = this->children();//获取子对象列表
        foreach (QObject *pobjGroup, pobjGroupList)//遍历子对象
        {
            QObjectList pobjItemList = pobjGroup->children();//获取每个子对象的子对象列表
