...
class CGxViewer : public QWidget
{
    Q_OBJECT

public:
    explicit CGxViewer(QWidget *parent = nullptr);
    ~CGxViewer();

    // **禁止拷贝构造和赋值**
       CGxViewer(const CGxViewer&) = delete;
       CGxViewer& operator=(const CGxViewer&) = delete;


    static CGxViewer& getInstance() {
         static CGxViewer instance;  // **C++11 及以后，线程安全的静态局部变量**
         return instance;
     }
    //对外提供接口
